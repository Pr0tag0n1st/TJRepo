...
int main()
{
	...
	ALLEGRO_BITMAP *mudPic = al_load_bitmap("mud.jpg");
	element air;
	element water;
	element fire;
	element earth;
	...
	mud.initElement(500, 100, 5, mudPic);
	...
	while (1)
	{
		...
		if (ev.type == ALLEGRO_EVENT_TIMER) {
			...
			if (earth.isLifted()) {
				earth.move(bouncer_x, bouncer_y);
			}
			if (water.isLifted()) {
				water.move(bouncer_x, bouncer_y);
			}
			if (fire.isLifted()) {
				fire.move(bouncer_x, bouncer_y);
			}
			if (mud.isLifted()) {
				mud.move(bouncer_x, bouncer_y);
			}
			...
			if (earth.clicked(bouncer_x, bouncer_y)) {
				if (earth.isLifted() == false)
					earth.liftUp();
				else
					earth.PutDown();
			}if (water.clicked(bouncer_x, bouncer_y)) {
				if (water.isLifted() == false)
					water.liftUp();
				else
					water.PutDown();
			}if (fire.clicked(bouncer_x, bouncer_y)) {
				if (fire.isLifted() == false)
					fire.liftUp();
				else
					fire.PutDown();
			}
			if (mud.clicked(bouncer_x, bouncer_y)) {
				if (mud.isLifted() == false)
					mud.liftUp();
				else
					mud.PutDown();
			}
